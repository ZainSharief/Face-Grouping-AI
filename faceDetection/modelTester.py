...
if __name__ == '__main__':
    ...
    model.load_weights('faceDetectionWeights/cp.ckpt')
    # Loads model weights from the file saved during training

    testImgpath = r'.\WIDERFaces\WIDER_test'
    # Directory to the test images folder
    testImages = os.listdir(r'.\WIDERFaces\WIDER_test')
    # Puts all the images from the folder into a list
    while True:
        img = random.choice(testImages)
        # Randomly picks an image from the folder
        img = os.path.join(testImgpath, img)
        # Combines the path to the folder and the image path
        ...
        bboxList = []
        # List to hold all bounding boxes for that image
        [bboxList.append(anchorBoxes[index]) for index in (bboxIndex.numpy())]
        # Creates the list of bounding box coordinates given the indexes

        bboxTensor = tf.convert_to_tensor(bboxList, dtype=np.float32)
        bboxTensor = tf.reshape(bboxTensor, (1, len(bboxList), 4))
        # Converts the bboxList into the required format for the bounding boxes to be drawn 

        bboxImage = tf.image.draw_bounding_boxes(preprocessedImg, bboxTensor, [[1.0, 0.0, 0.0]])
        bboxImage = tf.squeeze(bboxImage, axis=0).numpy()
        # Draws on the bounding boxes and puts it in the correct format to be an image
