...
# Function to pass images through the face detection model
    ...
    prediction = faceDetectionModel.predict(preprocessedImg, verbose=0)
    ...
    # Validates that there is at least one box to crop
        ...
        imageEmbeddings = faceRecognitionModel.predict(croppedImages, verbose=0) 
        # Passes each cropped image through the face recognition model to get the embedding

        imageEmbeddings = tf.unstack(imageEmbeddings, axis=0)
        # Splits up the tensor into a list of tensors with batch size 1

        return imageEmbeddings, tf.unstack(croppedImages * 255)
...
faceDetectionModel = SSD()
# Creates an instance of the face detection model
faceDetectionModel.load_weights('faceDetectionWeights/cp.ckpt')
# Loads model weights from the file saved during training

faceRecognitionModel = faceNet()
# Creates an instance of the face recognition model
faceRecognitionModel.load_weights('faceRecognitionWeights/cp.ckpt')
# Loads model weights from the file saved during training
